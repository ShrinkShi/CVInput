...
“换行模拟方式”提供实验对照矩阵，默认使用 `pyautogui`。可选方式包括 `pynput`、`win32_scan`、`win32_vk`、`pyautogui` 和 `keyboard`，便于在微信等特定目标输入框中逐项对照。普通文字字符输入仍使用 `pynput.keyboard.Controller().type(char)`。

`win32_scan` 使用 `SendInput` 扫描码方式，`win32_vk` 使用虚拟键方式，`pyautogui` 与 `keyboard` 使用各自的组合键注入路径。部分 Electron、Chromium、Qt 或聊天输入框对组合键解释较严格，可能需要在这些方式之间对照验证。
...
- 换行行为调试日志：记录输入过程中遇到换行符时采用普通 `Enter` 还是 `Shift+Enter`，以及按键按下/释放顺序。

开发者调试窗口提供“导出调试日志”和“清空调试日志”。导出文件为 UTF-8 文本，包含当前开发者模式状态、分类开关状态和最近的调试记录。

## 恢复默认设置

设置弹窗中提供“恢复默认设置”按钮，会恢复输入快捷键、热键开关快捷键、热键开关、默认 `30ms` 输入间隔、自动监听剪贴板、输入后清空、关闭到托盘、开机自启、透明度、语言、置顶状态、空剪贴板保护、输入法切换保护、换行输入策略及默认 `pyautogui` 换行模拟方式、记住设置、子窗口关闭行为、多槽位设置和开发者调试设置。
